...
function myAPI(url) {
    return $.ajax({
        beforeSend: function(request) {
            request.setRequestHeader("Client-Id", "k4j3nkws4mwx90yqfa3mlpn0i0udom");
            request.setRequestHeader("Authorization", "Bearer t4cc52i5bvdujcq70b9wnlkuyydkuu");
        },
        dataType: "json",
...
Chat = {
    info: {
        ...
        bots: ['streamelements', 'streamlabs', 'nightbot', 'moobot', 'fossabot', 'spanixbot'],
    ...
    write: function(nick, info, message) {
        if (info) {
            ...
            // custom cristianghost color
            if (info.subscriber !== "0"){
                $message.addClass('subscriber_message');
                $username.addClass('subscriber_username');
